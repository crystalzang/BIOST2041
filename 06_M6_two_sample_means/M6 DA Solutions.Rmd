...
### Hypothesis
$H_0 = 0$: The population mean difference in headache severity score between baseline and 1 year is zero.
...
Given that the p-value is smaller than our chosen significance level of `0.05` we decide that there is sufficient evidence to **reject the null hypothesis** and assume that the true population difference is not equal to 0.
...
The above results give us Test Statistic of `r method3$statistic` which is not too large, but still indicates a meaningful difference between the expected null value of the difference of means between the two groups. The p-value indicates that we have a probability of `r method3$p.value` for getting the observed result under the assumption that the $H_0$ is true. This is far below our significance level of 0.05 and thus indicates we have sufficient evidence to **reject the null** hypothesis. 
